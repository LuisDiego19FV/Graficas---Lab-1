...
class bmpImage:
	...
	# returns: 0 on success
	def glPolyFiller(self, vertices):
	...
	# Define glObjReader(). Makes BMP out of a .obj
	# Return 0 on success
	def glObjReader(self,objectName,scale,translate):
		...
		for face in faces:
			for i in range(len(face)-1):
				x1 = vertices[face[i][0]-1][0]/scale + translate
				y1 = vertices[face[i][0]-1][1]/scale + translate
				x2 = vertices[face[i+1][0]-1][0]/scale + translate
				y2 = vertices[face[i+1][0]-1][1]/scale + translate

				if i != len(face)-2:
					self.glLine(x1,y1,x2,y2)
				else:
					self.glLine(x1,y1,vertices[face[0][0]-1][0]/scale + translate,vertices[face[0][0]-1][1]/scale + translate)
